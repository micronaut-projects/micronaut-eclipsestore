...
@Controller("/${" + EclipseStoreRestControllerConfigurationProperties.PREFIX + ".path:" + EclipseStoreRestControllerConfigurationProperties.DEFAULT_PATH + "}")
...
public class EclipseStoreRestController {
    ...
    private final EclipseStoreRestService service;

    /**
     * A default controller to expose the expected EclipseStore REST API per storage manager.
     *
     * @param service the service to query EclipseStore via the relevant {@link StorageRestAdapter}
     */
    public EclipseStoreRestController(EclipseStoreRestService service) {
