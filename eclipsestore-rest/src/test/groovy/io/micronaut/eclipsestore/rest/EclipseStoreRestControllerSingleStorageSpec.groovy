...
class EclipseStoreRestControllerSingleStorageSpec extends Specification implements TestPropertyProvider {
    ...
    private ViewerStorageFileStatistics stats() {
        read("/eclipsestore/maintenance/filesStatistics", ViewerStorageFileStatistics)
    }

    @NonNull
    private RootObject root() {
        read("/eclipsestore/root", RootObject)
    }

    @NonNull
    private String dictionary(String manager = 'people') {
        httpClient.toBlocking().retrieve("/eclipsestore/dictionary")
    }

    @NonNull
    private ViewerObjectDescription object(String id) {
        return read("/eclipsestore/object/$id", ViewerObjectDescription)
    }
