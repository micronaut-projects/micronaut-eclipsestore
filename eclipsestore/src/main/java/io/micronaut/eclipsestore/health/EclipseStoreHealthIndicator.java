...
@Requires(property = HealthEndpoint.PREFIX + "." +  EclipseStoreHealthIndicator.ECLIPSE_STORE_PREFIX + ".enabled", value = StringUtils.TRUE, defaultValue = StringUtils.TRUE)
public class EclipseStoreHealthIndicator implements HealthIndicator {

    public static final String ECLIPSE_STORE_PREFIX = "eclipsestore";
    private static final String DOT = ".";
    private final Map<String, StorageManager> storageManagerMap = new ConcurrentHashMap<>();

    public EclipseStoreHealthIndicator(BeanContext beanContext) {
        ...
        return Flux.fromIterable(storageManagerMap.entrySet())
            .map(namedBean -> {
                ...
                return HealthResult.builder(
                        String.join(DOT, ECLIPSE_STORE_PREFIX, namedBean.getKey()),
    ...
    @NonNull
    private static EclipseStoreHealth healthOfManager(@NonNull StorageManager manager) {
        return new EclipseStoreHealth(manager.isStartingUp(),
            manager.isRunning(),
            manager.isActive(),
